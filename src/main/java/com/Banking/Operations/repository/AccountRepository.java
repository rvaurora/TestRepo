...
public class AccountRepository {
	...
	public void createAccount(Account obj) {
		String sql = "CREATE TABLE Account ( " + " CustomerID  varchar   not NULL PRIMARY KEY  " + "  " + " , " + " "
				+ "  CustomerName  varchar (255)  " + " , " + " " + "  AccountType  varchar (255)  " + " , " + " "
	...
	public void insertAccount(Account obj) {
		jdbcTemplate.execute(
				"insert into Account ( \"CustomerID\" ,  \"CustomerName\" ,  \"AccountType\" ,  \"CustomerAddress\"  ) values ('"
						+ obj.getCustomerID() + "', '" + obj.getCustomerName() + "', '" + obj.getAccountType() + "', '"
	...
	public int updateAccount(Account obj) {
		...
		int rows = jdbcTemplate.update(query, obj.getCustomerID(), obj.getCustomerName(), obj.getAccountType(),
				obj.getCustomerAddress(), obj.getCustomerID());
		return rows;
	}
	...
	public Account getAccount(Account obj) throws SQLException, DataAccessException {
		...
		return jdbcTemplate.query(query, new ResultSetExtractor<Account>() {
			public Account extractData(ResultSet rs) throws SQLException, DataAccessException {
				...
				if (rs.next()) {
					object.setCustomerID(rs.getLong("CustomerID"));
					object.setCustomerName(rs.getString("CustomerName"));
					object.setAccountType(rs.getString("AccountType"));
					object.setCustomerAddress(rs.getString("CustomerAddress"));
				}
				return object;
			}
		});
	...
	public List<Account> getAllAccount() {
		...
		for (Map<String, Object> row : rows) {
			Account object = new Account();
			object.setCustomerID((long) row.get("CustomerID"));
			object.setCustomerName((String) row.get("CustomerName"));
			object.setAccountType((String) row.get("AccountType"));
			object.setCustomerAddress((String) row.get("CustomerAddress"));
